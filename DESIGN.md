...
**Body & Display Font:** Pretendard (Variable, 자체 호스팅 · fallback: system-ui, sans-serif)
**Data:** 시간표 숫자는 Pretendard + `tabular-nums`로 정렬. Pretendard가 한글·라틴·숫자를 단일 패밀리로 감당하므로 별도 mono 폰트를 두지 않는다.

**Character:** 하나의 잘 튜닝된 한글 산세리프(Pretendard)가 제목·본문·라벨·데이터를 모두 감당한다. 한글 가독성이 뛰어나고 라틴·숫자도 정갈해, 고령·야외 사용자의 한국어 UI를 또렷하게 받친다. 제품 UI답게 display/body 페어링을 두지 않는다. 대비는 굵기와 크기로, 소음은 만들지 않는다.
...
- **Background:** 흰 표면. 다음-출발 블록만 방향색 가로 그라데이션(출발 `blue-50→sky-50`, 도착 `teal-50→teal-100`) — 배경 한정, 텍스트 그라데이션 아님. 도착은 틸 단일 계열로만(초록 운항상태색과 겹치지 않게).
...
- **Don't** 라벨·버튼·데이터에 display 폰트를 쓰지 마라. Pretendard 하나로 굵기·크기 대비만.
