...
최근 완료: **결항 판정을 MTIS 운항구분(`nvg_se_nm`) 기반으로 전환**(기존 `nvg_stts_nm="결항"`만
보던 버그 수정 — 전국 미운항 267편 오표시 해결), **결항(기상)/비운항(선박검사·정비·휴항) 구분 표시**,
**부분 결항 표시**(일부 편만 결항인 노선의 개별 편·경유 라벨 유지), **인천 덕적도·대이작도 추가**,
완도-제주 운임 링크(한일고속). 그리고 **MTIS 코어를 `lib/mtis.ts` 공용 모듈로 추출**(ferry.ts·
regionFerry.ts 중복 제거) + settings/scripts 정리.
...
- [x] 다지역 확장 — 포항(울릉 도동·사동+KTX)·목포(제주·홍도·흑산·비금도초·가거)·인천(백령·연평·**덕적·대이작**)
