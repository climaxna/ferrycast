...
**완도 생활 항로 3종**(여서도·모도·덕우도), **인천 항로 5종**, **첫 유료 광고 게재**(인천 36스테이,
`AdArea`+`until` 자동 하차+GA4 `ad_click`), 광고 위치를 노선 목록 중간으로 이동.

> ⚠️ **미해결 — Vercel 플랜**: Hobby는 광고 게재를 금지(약관상 상업적 사용)하며 사전 통보 없이
> 프로젝트를 내릴 수 있다. 유료 광고가 실제로 올라갔으므로 **Pro($20/월) 전환 필요**.
> ⚠️ **인천 36스테이 `until: "20260913"`** 은 월 단위 가정으로 넣은 임시값 — 실제 계약 기간 확인 필요.
> 🐞 **미해결 버그 — 빌드 시 MTIS 실패가 페이지에 굳는다**: `next build` 프리렌더 중 MTIS 호출이
> 실패하면 그 결과가 캐시에 박혀 **배포 후 최대 5~10분(revalidate) 동안 해당 지역이 정적 fallback
> ("참고 시간표")으로 서비스된다.** 결항 정보가 안 나온다는 뜻이라 이 앱에선 치명적이다.
> 재현율이 낮지 않다(점검 중 3회 관측, 매번 다른 지역). 다음 작업 후보 1순위.
...
- [x] 다지역 확장 — 목포(제주·홍도·흑산·비금도초·가거·**외달도**+생활항로)·인천(백령·덕적·대이작 + **장봉·연평·굴업·풍도·울도**)
- [x] **완도 생활 항로** — 여서도·모도·덕우도(순환항로 매칭)
- [x] **목적지 허브 탭** — **제주**(목포·완도·진도·녹동·삼천포) / **울릉도**(묵호·강릉·포항·영일만, 구 포항 탭 흡수)
...
**완도**(메인 `/`, `lib/ferry.ts` — 완도 전용 매핑 + 약산권):
- 완도 → 제주 (한일골드스텔라·실버클라우드 직항 + **송림블루오션 추자도 경유**, MTIS 실시간 ✅)
...
- **목포** → 제주·홍도·흑산도·비금도초·가거도·**외달도**(순회) + 신안·진도 생활 항로 섹션
- **인천** → 백령도·덕적도·대이작도 + 장봉·연평·굴업·풍도·울도(`islandHops`)
- **울릉도**(`/ulleung`, 구 `/pohang` 흡수) ← 묵호·강릉·포항·영일만신항. 포항역 KTX + **울릉도→독도**
- **제주**(`/jeju`) ← 목포·완도·진도·녹동·삼천포

> **목적지 허브 탭 (`inbound: true`)**: 제주·울릉도는 방향이 반대다 — 출발항 N곳 → 목적지 1곳.
> routeGroup의 `label`이 목적지가 아니라 **출발지**를 뜻하므로 카드 from/to·요약 라벨·탭 문구를 모두 뒤집는다
> (탭: "제주 가는 배 / 제주 나오는 배"). `RouteItem`이 이미 `from → to` 렌더를 지원해 전용 컴포넌트는 없다.
> ⚠️ **완도·목포 탭의 제주는 빼지 않는다** — 완도 최대 노선이라 없애면 원칙 #1에 어긋난다. 두 탭은 질문이 다르다.
> ⚠️ **포항 탭은 없어졌다.** 노선이 전부 울릉도행이라 울릉도 탭이 완전히 포함한다.
> `/pohang` → `/ulleung` 영구 리다이렉트(next.config.ts) — QR·블로그 링크 보호.

> **순환항로 매칭 (`seawayKeywords`)**: 일반 노선인데 MTIS에 순환(oport=dest=본항)으로 등록된 편이 있다
> (목포 가거도 뉴엔젤호, 외달도 전 구간, 완도 모도·덕우도). dest로는 안 잡혀 **같은 섬인데 일부 편만 표시되는
> 누락**이 생긴다. → routeGroup에 `seawayKeywords`(항로명 매칭)를 더해 한 카드로 합친다.
> **출발편에만 적용** — 순환은 왕복이 한 편이라 도착 탭에 또 실으면 중복이다.
> 완도는 `ferry.ts`의 `WANDO_CIRCULAR`가 같은 일을 한다.
> 도착편이 아예 없는 노선(모도·덕우도·외달도)은 `routeNote`로 상세 화면에 이유를 설명한다.
...
> ✅ **인천 연평도·굴업도·풍도도 `islandHops`로 실시간화 완료** (2026.08). 함께 **장봉도**(삼목항 출발이라
> `depPortKeywords: ["인천"]`에 안 걸리던 하루 14편 노선)와 **울도**(덕적도 진리항 출발)도 추가.
> ⚠️ **islandHops는 도착 탭에 실리지 않는다** — 왕복을 보여주려면 장봉도처럼 역방향 항목을 따로 넣어야 한다
> (`jangbongdo` / `jangbongdo-return`). 또 엔진이 `noBooking: true`를 고정으로 넣어 **예약 버튼이 안 나온다**
> (연평·장봉은 실제로 예매 가능할 수 있어 확인 필요).
...
- **응답 필드** (`response.body.items.item[]`):
  ...
  - `dest_nm`: 도착항 이름
  - `psnshp_nm`: 여객선명
  - `nvg_se_nm` / `nvg_se_cd`: **운항구분 — 실제 운항 여부를 가르는 권위 필드**. `정상`(1)·`증선`(2)·`증회`(3)·`대기/지연`(6)=운항 / `비운`(4, 선박검사·정비·휴항)·`통제`(5, 풍랑 등 기상)=미운항. 결항 판정은 반드시 이 필드로 (`lib/mtis.ts`의 `isCancelled()`)
    - `대기/지연`(6): 해무 등 통제 해제 후 **지연 재개한 배** (2026.07 약산 평화페리9호 실측 — 통제가 풀릴 때 전 선박이 아니라 한 척만 먼저 재개하면, 그 배만 6이 되고 나머지 편은 5(통제) 유지 → "정상-결항-결항-정상" 편별 패턴은 오류가 아니라 감축 운항의 실상). 운항 취급이 맞음
    - UI는 성격을 구분해 표시: **통제(5)=`결항`(rose, 기상·오늘 한정)** vs **비운(4)=`비운항`(amber, 선박검사 등 계획·다일)**. `isSuspended()`/`cancelKind`/`cancelledTimes[].suspended`로 전달 (theksa 공식도 선박검사편은 "비운항"으로 표기)
  - `nvg_stts_nm`: 진행상태 (`출항전`·`운항중`·`완료`). ⚠️ **`"결항"` 값은 거의 안 나온다** — 이것만 보면 미운항편을 통째로 놓친다 (2026.07 포항 사동 뉴씨다오펄 23시편 오표시 버그: 전국 미운항 267편 중 `nvg_stts_nm="결항"`은 0건이었음)
  - `cntrl_rsn_nm`(통제사유)·`nnavi_rsn_nm`(미운항사유): 결항 **사유 표시용**. ⚠️ `nnavi_rsn_nm`(선박검사 등)은 **정상 운항편에도 붙는 노이즈** — 운항 여부 판정에 쓰지 말 것(정상 배를 숨김). 판정은 `nvg_se_nm`만
...
1. **구글 애드센스 신청** + 광고 슬롯 레이아웃 (현재 카카오 AdFit·쿠팡만 연동)
2. **3종 환경 테스트**: iOS Safari / Android Chrome / PC — 레이아웃 이상 없는지 확인
3. (선택) 인천 연평도 실시간화 — 순환항로 `seawayKeywords` 매칭 엔진 (위 "알려진 한계" 참고)
4. (선택) 완도-제주 직항/추자경유 요금 분리 표시 (현재 한일고속 링크 하나로 통합)
