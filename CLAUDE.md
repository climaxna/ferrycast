...
- **응답 필드** (`response.body.items.item[]`):
  ...
  - `dest_nm`: 도착항 이름
  - `psnshp_nm`: 여객선명
  - `nvg_se_nm` / `nvg_se_cd`: **운항구분 — 실제 운항 여부를 가르는 권위 필드**. `정상`(1)·`증선`(2)·`증회`(3)=운항 / `비운`(4, 선박검사·정비·휴항)·`통제`(5, 풍랑 등 기상)=미운항. 결항 판정은 반드시 이 필드로 (`lib/ferry.ts`·`regionFerry.ts`의 `isCancelled()`)
