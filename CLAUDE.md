...
최근 완료: **첫 유료 광고 게재 시작(인천 36스테이)** — `LOCAL_ADS`를 실서비스에 연결하고
(`AdArea`), 계약 만료일(`until`)이 지나면 자동으로 내려가는 안전장치와 **GA4 `ad_click` 클릭 집계**를
붙였다. `/ads` 게재 예시를 실물 배너 4종(특산물·펜션·식당·카페)으로 교체, `/ads/preview`
광고주용 목업 페이지 추가. 그리고 **인천 항로 5종 확장** — 장봉도(하루 14편, 인천 최다)·
굴업도·풍도·울도 신규 + **연평도 실시간화**(순환항로라 정적 fallback만 나오던 문제 해결).
...
- [x] 다지역 확장 — 포항(울릉 도동·사동+KTX)·목포(제주·홍도·흑산·비금도초·가거)·인천(백령·덕적·대이작 + **장봉·연평·굴업·풍도·울도**)
...
│       ├── regionFerry.ts       ← 다지역(포항·목포·인천) 항로 — config/regions 기반 (mtis.ts 소비)
