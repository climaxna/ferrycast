    ...
    <div className="fixed inset-0 z-[9999] flex flex-col bg-white" style={{ height: "100dvh" }}>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="mx-auto max-w-lg space-y-5 px-4 py-5">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {cx.map((c) => (
                      <span key={c.time} className="inline-flex items-center gap-1.5 rounded-lg bg-white px-2.5 py-1.5 shadow-sm">
                        ...
                        {/* 선박명 — 교대 운항 노선에서 편별 결항 차이(한 척만 재개 등)의 이유를 드러냄 */}
                        {c.ship && <span className="text-[11px] font-medium text-slate-400">{c.ship}</span>}
          ...
          {/* 운임 확인과 예매가 같은 공식 예약 흐름으로 이어져, 행동을 하나로 통일한다. */}
          {!route.noBooking && (
            <a
              href="https://island.theksa.co.kr/page/booking"
              target="_blank"
              rel="noopener noreferrer"
              className="flex w-full items-center justify-center gap-1.5 rounded-xl bg-blue-600 py-3 text-sm font-bold text-white transition-opacity hover:opacity-90 active:opacity-80"
            >
              예약하기
