...
export default function RegionTidalForecastDetail({ days, regionName, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex flex-col bg-white" style={{ height: "100dvh" }}>
      <div className="flex items-center gap-3 border-b border-slate-100 px-4 py-4">
        ...
        <div>
          <h2 className="text-lg font-bold tracking-tight text-slate-900">{regionName} 5일 조석 예보</h2>
          <p className="text-xs text-slate-400">국립해양조사원(KHOA) {regionName} 관측소 기준</p>
        </div>
      </div>
              ...
              return (
                <div
                  ...
                  className={`flex flex-col items-center rounded-xl px-1 py-2 ${
                    isNext ? "bg-blue-50 ring-1 ring-blue-200" : "bg-slate-50"
                  } ${isPast ? "opacity-45" : ""}`}
                >
                  <span className={`text-[11px] font-bold ${isHigh ? "text-blue-600" : "text-slate-500"}`}>
                    {isHigh ? "▲ 만조" : "▼ 간조"}
                  </span>
