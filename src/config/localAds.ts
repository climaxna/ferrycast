...
export const LOCAL_ADS: LocalAd[] = [
  {
    ...
    // 지인 업체라 계약 만료 압박이 없음(2026-09-16 운영자 확인) — 실제 계약 종료일 대신
    // PREVIEW_AD와 같은 방식으로 먼 미래 날짜를 넣어 사실상 만료 없이 유지한다.
    // (`until` 자체를 없애지 않는 이유: "돈 안 받는데 계속 걸려 있는 사고" 방지용 타입 강제를
    // 이 광고 하나 때문에 느슨하게 만들지 않기 위해서다 — 내릴 일이 생기면 이 값을 오늘 날짜로.)
    until: "20991231",
    title: "36스테이",
