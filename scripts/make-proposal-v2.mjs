...
const SITE_PHOTOS = [
  ["qr_wando_1.jpg", "완도버스터미널 — 선박운항시간표 옆"],
  ["qr_wando_2.jpg", "완도버스터미널 — 대합실"],
...
const CONCERNS = [
  ...
  ["광고가 공익성을 해치지 않나?", "광고·특산물 코너는 핵심 정보를 모두 확인한 화면 맨 아래에만 있습니다. 광고 자리도 외부 광고망 대신 완도 소상공인 소개 자리로 운영해 지역에 환원되는 구조입니다."],
  ["실제로 쓰이고 있다는 근거가 있나?", "2026년 7월 기준 최근 일주일 평균 하루 150명 이상이 방문하고 있습니다. 별도 광고 없이 현장 QR 부착과 온라인 소개만으로 얻은 수치이며, 최근 들어 방문자가 눈에 띄게 늘고 있습니다. 현장에서도 이용자가 QR을 스캔해 확인하는 모습을 직접 확인했습니다. 정식 부착 이후에는 일간 접속자 추이를 정기적으로 완도군청에 공유해 드리겠습니다."],
  ["운영자가 그만두면 끊기지 않나?", "공공 API 기반이라 구조가 단순합니다. 군과 협의해 지속 방안(자료·운영 인수 등)을 열어 두겠습니다."],
]

const REQUESTS = [
  ["1", "정식 부착 허가", "완도버스터미널 등 완도 공공장소에 내구성 있는 QR 안내물 정식 부착 허가 (현재 임시 시범물 → 정식 안내물로 교체)"],
...
function buildHtml() {
  ...
  * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  ...
  <p>민간이 공공데이터로 만든 서비스를 행정이 협력으로 받아들인 선례도 이미 있습니다. 고등학생이 만든 ‘서울버스’ 앱은 이용 수요가 확인되자 지자체가 데이터 협력으로 방향을 바꿨고, 시민 개발자들이 만든 공적마스크 앱과 개인안심번호는 정부가 데이터를 제공하며 공식 채택했습니다. FerryCast는 처음부터 공공 오픈 API를 정식 절차로 사용하고 있어, 데이터 문제 없이 곧바로 협력할 수 있습니다.</p>
  ...
  <p>서비스의 효용을 직접 확인하고자, 자비로 안내물을 제작해 완도버스터미널과 완도 진입 경로의 거점인 해남종합버스터미널에 시범 부착해 보았습니다. 실제로 완도버스터미널에 게시된 선박운항시간표 바로 옆에서 이용자들이 휴대폰으로 스캔해 배 시각과 결항을 확인합니다.</p>
  ...
  <h3>관광객 만족</h3>
  <p>결항을 미리 확인해 일정 차질을 줄입니다. “정보를 쉽게 찾을 수 있는 완도”라는 인상을 남깁니다. 실제로 방문자 대다수가 완도 밖에서 검색·블로그를 통해 유입되고 있어, 여행 전에 배편 정보를 찾는 관광객에게 이미 닿고 있음이 확인됩니다.</p>
  <h3>군의 부담 없는 협력</h3>
...
async function makePdf() {
  ...
  // PLAYWRIGHT_CHROMIUM 지정 시 해당 바이너리 사용 (버전 불일치 환경 대응), 미지정 시 기본 탐색
...
async function makeDocx() {
  const children = [
    ...
    body("민간이 공공데이터로 만든 서비스를 행정이 협력으로 받아들인 선례도 이미 있습니다. 고등학생이 만든 '서울버스' 앱은 이용 수요가 확인되자 지자체가 데이터 협력으로 방향을 바꿨고, 시민 개발자들이 만든 공적마스크 앱과 개인안심번호는 정부가 데이터를 제공하며 공식 채택했습니다. FerryCast는 처음부터 공공 오픈 API를 정식 절차로 사용하고 있어, 데이터 문제 없이 곧바로 협력할 수 있습니다."),
    ...
    h1("8. 추진 방안과 일정"),
    h2("이미 현장에 시범 부착했습니다"),
    body("서비스의 효용을 직접 확인하고자, 자비로 안내물을 제작해 완도버스터미널과 완도 진입 경로의 거점인 해남종합버스터미널에 시범 부착해 보았습니다. 실제로 완도버스터미널에 게시된 선박운항시간표 바로 옆에서 이용자들이 휴대폰으로 스캔해 배 시각과 결항을 확인합니다."),
    ...
    h2("관광객 만족"),
    body("결항을 미리 확인해 일정 차질을 줄입니다. “정보를 쉽게 찾을 수 있는 완도”라는 인상을 남깁니다. 실제로 방문자 대다수가 완도 밖에서 검색·블로그를 통해 유입되고 있어, 여행 전에 배편 정보를 찾는 관광객에게 이미 닿고 있음이 확인됩니다."),
    h2("군의 부담 없는 협력"),
